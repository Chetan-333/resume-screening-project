...
- Web interface for uploading resumes/JDs and viewing ranked results

## Tech Stack

- **Backend:** FastAPI
- **Frontend:** Jinja2 templates + HTML/CSS (Tailwind CSS) + minimal JS
...
├── requirements.txt
├── .gitignore
├── data/               # sample resumes/job descriptions (no real PII)
├── src/
...
| POST | `/rank` | Accepts resumes + job description, returns ranked results |
| GET | `/results/{job_id}` | View ranked results for a submission |
